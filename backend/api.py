...
def _normalize_ticket_filters(params):
    ...
    the retired S1–S4 and P1–P4 codes and case variants, so a bookmarked URL or an
    older client still resolves instead of silently returning an empty list —
    which is exactly what happened when two layers disagreed about the vocabulary."""
...
def approve_ticket(ticket_id: str):
    ...
    if not external_id:
        log.warning(
            "post-approval comment/transition skipped for %s: missing external_id",
            ticket_id,
        )
    else:
        try:
            ...
            log.warning(
                "post-approval comment/transition failed for %s (%s): %s",
                ticket_id,
                external_id,
                exc,
            )
